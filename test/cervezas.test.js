...
//importar la aplicacion sobre la que quiero hacer tests
...
const Cervezas = require("../src/cervezasModel.js");
...
//antes de cada test limpiar la base de datos y volverla a crear limpia
beforeEach((done) => {
  Cervezas.deleteMany({})
    .then(() => {
...
describe("Recurso cervezas", () => {
  ...
  describe("Obtener una cerveza", () => {
    ...
    it("Devuelve un 404 si no existe su id", (done) => {
      request(app).get(`/api/cervezas/${idTest}`).expect(404).end(done);
    });

    it("Devuelve un 404 si no existe la palabra clave", (done) => {
      request(app).get(`/api/cervezas/search?q=perejil`).expect(404).end(done);
    });

    it("Devuelve un 404 si el id es erróneo", (done) => {
      request(app).get(`/api/cervezas/idErróneo`).expect(404).end(done);
    });
  });
  ...
  describe("Borrar una cerveza", () => {
    ...
    it("Devuelve un 404 si no existe", (done) => {
      request(app).delete(`/api/cervezas/${idTest}`).expect(404).end(done);
    });
    it("Devuelve un 404 si el id es erróneo", (done) => {
      request(app).delete(`/api/cervezas/idErróneo`).expect(404).end(done);
    });
  });
